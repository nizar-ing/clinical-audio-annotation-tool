...
**Icons — lucide-vue-next.** Added as a web dependency (explicit user approval). Lucide is a clean, consistent SVG icon set — 1 px stroke-based, zero runtime weight beyond the tree-shaken SVG components imported. Each icon is semantically paired to its context: `Mic` for the brand, `ListChecks` for the queue heading, `FileAudio` for audio file rows, `Loader2 + animate-spin` for loading states, `Inbox` for empty states, `SkipBack`/`SkipForward`/`Play`/`Pause`/`Keyboard` for the audio player controls, `ArrowLeft`/`Clock`/`Activity`/`Headphones` for the workspace header, `X` for the overlay close button, and per-status icons on Badge (`Upload`, `Ban`, `Unlink`, `Clock`, `CircleDot`, `CircleCheck`). No emoji, no Unicode glyphs, no image assets.
...
**Word alignment uses two tiers.** With speech regions available (decoded via `ffmpeg-audio-energy.adapter.ts`), tokens are distributed across regions proportionally to character length — method string `"energy-gated"`. Without regions (decode failed, empty audio), tokens are spread linearly across the full duration — method string `"proportional"`. The method string travels through the `Transcript` row and into the JSONL export unchanged, so a downstream consumer can filter by confidence tier. Both are labelled estimates; neither is presented as forced alignment.

**Span re-anchoring after transcript edits.** When `correctedText` changes, character offsets stored in `AnnotationSpan` become stale. The span-anchoring domain service finds all occurrences of the stored `anchorText` in the new text and picks the one whose old `startOffset` is closest. If the anchor text no longer appears at all, the span is flagged with `needsReview = true` and a badge appears in the workspace's side panel. Silently misplacing a span in data that becomes a training set is not an option — forcing human review is the safe choice. Re-anchoring runs inside the `UpdateCorrectedTranscriptHandler` so it is atomic with the text save.

**Transcript materialisation via cross-context port.** `import-transcripts.handler.ts` previously stored transcripts only through the seed script. A `CreateTranscriptPort` interface declared in `ingestion/application/ports/` allows the ingestion context to call the transcription context's `CreateTranscriptHandler` without importing from it. The composition root (`main/app.ts`) wires the dependency by adapting the transcription handler to the port shape — twelve lines, no framework, dependency direction preserved.

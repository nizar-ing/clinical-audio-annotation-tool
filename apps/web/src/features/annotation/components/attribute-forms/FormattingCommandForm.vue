...
const command = ref<'newline' | 'paragraph' | 'tab' | 'period' | 'comma'>('newline');
...
<template>
  ...
  >
    ...
    <label class="flex items-center gap-2 text-sm font-sans text-warm-700 normal-case tracking-normal">
      <input
        v-model="literal"
        type="checkbox"
        class="accent-sage-500"
      >
      Literal — the speaker is saying the words rather than issuing the command
    </label>
